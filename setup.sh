...
sudo apt-get install -y libnss3 libgbm1\
...
cd ./app
npm install
node ./utils/decode.js
export IPADDR=$(curl 'http://checkip.amazonaws.com')
echo 'export IPADDR=$(curl "http://checkip.amazonaws.com")' >> ~/.bashrc
source ~/.bashrc
