...
const publishAsync = (
  ...
  const messageLocations = ["40.495791,-74.254845", "38.933776,-77.040470", "42.236275,-71.185923"];
  
  setTimeout(() => {
    const payload = messageLocations[Math.floor(Date.now()) % 3];
    // Publish "payload" to the topic. qos=1 means at least once delivery.
...
const mqttDeviceDemo = (
  ...
  // The topic that this device will publish data to. The topic name is
  // required to be in the format below. The topic name must end in 'state' to
  // publish state and 'events' to publish telemetry. 
...
const publishAsyncGateway = (
  ...
  setTimeout(() => {
    ...
    const schedulePublishDelayMs = 5000;
    setTimeout(() => {
      const secsFromIssue = parseInt(Date.now() / 1000) - iatTime;
